...
class LobbyScene extends Phaser.Scene {
    ...
    createMainMenu() {
        ...
        // Cleanup button (for debugging/maintenance)
        const cleanupBtn = this.add.rectangle(0, 180, 300, 40, 0x795548)
            .setInteractive()
            .on('pointerdown', () => this.forceCleanup())
            .on('pointerover', () => cleanupBtn.setFillStyle(0x8D6E63))
            .on('pointerout', () => cleanupBtn.setFillStyle(0x795548));
        
        const cleanupText = this.add.text(0, 180, 'Clean Up Orphaned Rooms', {
            fontSize: '14px',
            fill: '#ffffff',
            fontFamily: 'Arial'
        }).setOrigin(0.5);
    ...
    async quickMatch() {
        try {
            ...
            console.log('🎯 LobbyScene: Starting quick match...');
            const roomCode = await multiplayerManager.quickMatch();
            this.currentRoomCode = roomCode;
            console.log('🎯 LobbyScene: Quick match completed, room code:', roomCode);
            ...
            if (playerCount > 1) {
                ...
                console.log('🎯 LobbyScene: Joined existing room with', playerCount, 'players');
            } else {
                this.showMessage('Room created! Waiting for players...', 0x4CAF50);
                console.log('🎯 LobbyScene: Created new room');
            }
        } catch (error) {
    ...
    async joinRoom(roomCode) {
        try {
            ...
            console.log('🚪 LobbyScene: About to join room', roomCode);
            await multiplayerManager.joinRoom(roomCode);
            this.currentRoomCode = roomCode;
            console.log('🚪 LobbyScene: Successfully joined room, calling showRoomInterface()');
            this.showRoomInterface();
            this.showMessage('Joined room!', 0x4CAF50);
            console.log('🚪 LobbyScene: Room interface shown, isInRoom =', this.isInRoom);
        } catch (error) {
    ...
    showRoomInterface() {
        console.log('🏠 LobbyScene: showRoomInterface() called');
        this.mainMenuContainer.setVisible(false);
        this.roomContainer.setVisible(true);
        this.isInRoom = true;
        console.log('🏠 LobbyScene: Set isInRoom =', this.isInRoom);
        ...
        console.log('🏠 LobbyScene: Room interface setup complete');
    }

    updateRoomDisplay(roomData) {
        console.log('🔍 LobbyScene updateRoomDisplay called:', {
        ...
        if (!roomData) {
            console.log('❌ No room data provided to updateRoomDisplay');
            return;
        }
        
        if (!this.isInRoom) {
            console.log('❌ Not in room, but checking if game is starting anyway...');
            // Fallback: Check if game is starting even if not properly in room interface
            if (roomData.status === 'starting') {
                console.log('🎮 LobbyScene detected game starting (fallback), calling handleGameStarting()');
        ...
        try {
            ...
            // Safely update players list text
            if (this.playersListText && this.playersListText.active) {
            ...
            if (roomData.isQuickMatch && roomData.autoStartTime) {
                ...
                if (secondsRemaining > 0) {
                    ...
                    if (this.autoStartText && this.autoStartText.active) {
        ...
        // Check if game is starting - this should always execute regardless of display errors
        if (roomData.status === 'starting') {
            console.log('🎮 LobbyScene detected game starting, calling handleGameStarting()');
            this.handleGameStarting();
        }
    }
    ...
    handleGameStarting() {
        console.log('🎮 LobbyScene handleGameStarting() called');
        this.showMessage('Game starting...', 0x4CAF50);
        
        // Transition to game scene after short delay
        this.time.delayedCall(2000, () => {
            console.log('🎮 LobbyScene transitioning to GameScene with UIScene');
            this.scene.start('GameScene', { gameMode: 'multiplayer', roomId: this.currentRoomCode, roomPlayers: multiplayerManager.getRoomPlayers() });
    ...
    async forceCleanup() {
        this.showMessage('Cleaning up orphaned rooms...', 0xFFFF00);
        
        try {
            await multiplayerManager.forceCleanupOrphanedRooms();
            this.showMessage('✅ Cleanup completed!', 0x4CAF50);
        } catch (error) {
            console.error('Cleanup failed:', error);
            this.showMessage('❌ Cleanup failed', 0xF44336);
        }
    }
