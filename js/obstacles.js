...
function createChunkedTrack(scene, trackWidth, groundTopY, groundSegmentHeight, seed = null) {
    const groundGroup = scene.physics.add.staticGroup();
    const wallsGroup = scene.physics.add.staticGroup();
    ...
    while (currentX < trackWidth - 1000) { // Leave space for finish zone
        ...
        // Pick a deterministic pattern from the difficulty category
        const availablePatterns = PATTERN_CATEGORIES[difficulty];
        const patternIndex = Math.floor(seededRandom() * availablePatterns.length);
        const patternKey = availablePatterns[patternIndex];
        ...
        // Add deterministic buffer between chunks
        const bufferWidth = 50 + Math.floor(seededRandom() * 100); // 50-150px buffer (deterministic)
        allTrackSegments.push({
            type: 'ground',
            start: currentX,
